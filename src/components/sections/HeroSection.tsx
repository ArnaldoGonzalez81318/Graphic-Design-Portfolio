...
const HeroSection = ({ hero }: HeroSectionProps) => {
  ...
  return (
    <section id="home" className="relative overflow-hidden bg-night">
      ...
      <div className="container-grid relative mx-auto flex w-full max-w-6xl flex-col items-center gap-8 py-12 sm:gap-12 sm:py-20 xl:grid xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] xl:items-start xl:gap-16 xl:py-24 2xl:gap-20 2xl:py-28">
        <motion.div
          initial={{ opacity: 0, y: 32 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: 'easeOut' }}
          className="w-full max-w-3xl space-y-6 text-center sm:space-y-8 xl:max-w-2xl xl:text-left"
        >
          <div className="mx-auto max-w-2xl space-y-4 xl:mx-0">
            ...
            <h1 className="text-[1.82rem] font-display font-semibold leading-[1.06] text-white max-[359px]:text-[1.66rem] sm:text-4xl sm:leading-tight md:text-5xl xl:text-6xl 2xl:text-7xl">
              {hero.headline}
            </h1>
            <p className="mx-auto max-w-2xl text-[0.95rem] leading-relaxed text-muted max-[359px]:text-[0.9rem] sm:text-lg md:text-xl xl:mx-0">
              {hero.description}
            </p>
          </div>
          ...
          <div className="grid w-full gap-4 text-sm text-muted min-[420px]:grid-cols-2">
        ...
        <motion.div
          ...
          className="relative w-full max-w-lg rounded-[2rem] border border-white/5 bg-gradient-to-br from-white/10 via-transparent to-white/5 p-5 text-white shadow-[0_30px_120px_-60px_rgba(56,189,248,0.55)] sm:max-w-xl sm:rounded-[2.5rem] sm:p-8 xl:ml-auto xl:max-w-lg"
        >
          <div className="space-y-6">
            <div className="rounded-3xl bg-white/5 p-6 text-left shadow-inner">
              ...
              <p className="mt-3 text-xl font-display text-white sm:text-2xl">
                Building brand systems, editorial layouts, and campaign assets that stay cohesive from first concept to final production.
              </p>
            </div>
            <div className="grid gap-3 min-[420px]:grid-cols-2 sm:gap-4 lg:grid-cols-2 xl:grid-cols-3">
              {hero.metrics.map((metric) => (
                ...
                >
                  ...
                  <p className="mt-2 text-[0.68rem] uppercase leading-relaxed tracking-[0.2em] text-muted max-[359px]:tracking-[0.16em]">
                    {metric.label}
                  </p>
                </div>
              ))}
